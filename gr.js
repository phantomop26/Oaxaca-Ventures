...
async function scrapeGoogleReviews(places) {
    ...
    for (const place of places) {
        ...
        const moreButtonSelector = '.w8nwRe.kyuRq';
        const moreButtonExists = await page.$(moreButtonSelector);
        await delay(1000);  
        ...
        const placeDetails = await page.evaluate(() => {
            ...
            const neighborhoodRating = document.querySelector('.gm2-headline-3')?.innerText || null;
            const neighborhoodReviewheadText = document.querySelector('.SsuqXb')?.innerText || null;
            const neighborhoodReviewText = document.querySelector('.MmD1mb')?.innerText || null;  
            const neighborhood = document.querySelector('.wh8dOb.kyuRq');
            const neighborhoodText = neighborhood ? neighborhood.textContent : null;
            const priceElement = document.querySelector('.OyY9Kc span');
            ...
            return {
                ...
                neighborhoodRating, 
                neighborhoodText, 
                neighborhoodReviewheadText,
                neighborhoodReviewText,
            };
        });
        ...
        try {
            ...
            console.error(`Error navigating to the About section for ${place}:`, error);
            continue; 
        }

        const aboutDetails = await page.evaluate(() => {
            const aboutSectionText = document.querySelector('.HeZRrf')?.innerText || null;
            const tagsElements = document.querySelectorAll('.CK16pd.dc6iWb'); 
            const availtags = Array.from(tagsElements).map(element => element.getAttribute('aria-label'));
            const tagsElem = document.querySelectorAll('.CK16pd.fyvs7e'); 
            const unavailtags = Array.from(tagsElem).map(element => element.getAttribute('aria-label'));        
            return {
                about: aboutSectionText,
                availabletags: availtags,
                unavailabletags: unavailtags,
            };
        });
        ...
        try {
            ...
            await page.screenshot({ path: `screenshot-${place.replace(/\s+/g, '_')}.png` });
            continue; 
        }
        ...
        while (true) {
                ...
                return Array.from(reviewElements).map(review => {
                    ...
                    if (rating && text) {
                        return {
                            ...
                            photoUrls, 
                            userProfilePicture,
                            userhistory: Array.from(userhistory).map(item => item.getAttribute('data-href')), // Extract userhistory properly
                        };
                    }
                    return null;
            ...
            // Ensure newReviews is defined before proceeding
